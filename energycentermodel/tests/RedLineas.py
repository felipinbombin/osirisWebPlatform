...
class Linea:
    ...
    def CargarDatos(self, Data):
        ...
        for SERID, SERData in Data['Atributos']['SERData'].items():
            ...
            newSER = self.addSER(Vias, SERID, SERData['pos'], SERData['Vac'], SERData['Vdc'], SERData['R'],
                                 SERData['TermID'])
    ...
    def simular(self, t_index=None, *varargin):
        ...
        if t_index == None:
            ...
            for elemento in self.Elementos:
                ...
                elemento.Results[datetime.strftime(self.fechas[t_index], '%Y-%m-%d %H:%M:%S')] = {
                    'V': elemento.Nodo.V.item(), 'P': elemento.P[t_index]}
    ...
    def get_charts(self):
        ...
        chart3_data_label = []
        for elemento in self.Elementos:
            chart1_data.append([resultados['V'] for _, resultados in elemento.Results.items()])
            chart1_data_label.append(elemento.ID)

            if isinstance(elemento, SER):
                chart2_data.append([resultados['P'] / 1000000 for _, resultados in elemento.Results.items()])
                chart2_data_label.append(elemento.ID)

            if isinstance(elemento, PV):
                chart3_data.append([P / 1000000 for P in elemento.P])
                chart3_data_label.append(elemento.ID)
